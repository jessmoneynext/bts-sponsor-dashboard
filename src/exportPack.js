...
import { isRecoveredSponsor, RECOVERY_FOOTNOTE } from './recovery.js';
...
const NOTE_STYLE = { font: { italic: true, sz: 9, color: { rgb: '595959' } }, alignment: { wrapText: true, vertical: 'top' } };
...
export const exportSponsorPack = (sponsor, data, enrichmentMap) => {
  ...
  const showRecoveryNote = isRecoveredSponsor(sponsor.name);
  ...
  if (showRecoveryNote) {
    sumRows.push([]);
    sumRows.push([RECOVERY_FOOTNOTE]);
  }
  ...
  if (showRecoveryNote) {
    const noteRow = sumRows.length; // 1-indexed Excel row matches array length after empty row
    const noteRef = `A${noteRow}`;
    if (sumWs[noteRef]) sumWs[noteRef].s = NOTE_STYLE;
    sumWs['!merges'].push({ s: { r: noteRow - 1, c: 0 }, e: { r: noteRow - 1, c: 1 } });
    if (!sumWs['!rows']) sumWs['!rows'] = [];
    sumWs['!rows'][noteRow - 1] = { hpx: 80 };
  }
  ...
  // Extra "Source" column flags any rows recovered from the misconfigured stage scanner
  const sessHeaders = ['Check-in time', 'Attendee name', 'Job title', 'Company', 'Email', 'Phone', 'Source', ...ENR_HEADERS];
  if (sessionBlocks.length === 0) {
    ...
    for (const { session, rows } of sessionBlocks) {
      ...
      if (rows.length === 0) sessRows.push(['No scan data for this session yet', ...Array(sessHeaders.length - 1).fill('')]);
        ...
        r._recovered ? 'Recovered (stage scanner)' : 'Live scan',
        ...enrCols(r.participant_email, enrichmentMap),
      ]);
    }
  }
  const sessWs = XLSX.utils.aoa_to_sheet(sessRows);
  sessWs['!cols'] = [{ wch: 20 }, { wch: 22 }, { wch: 30 }, { wch: 22 }, { wch: 32 }, { wch: 16 }, { wch: 24 },
                     ...Array(ENR_HEADERS.length).fill({ wch: 28 })];
